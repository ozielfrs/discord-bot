...
function mentionToName(str, e) {
	...
	while (str.includes(rlS, pos)) {
		snwflk = str
			.slice(str.indexOf(rlS, pos), str.indexOf(`>`, str.indexOf(rlS, pos)))
			.replace(rlS, ``)
	...
	while (str.includes(usrS, pos)) {
		snwflk = str
			.slice(str.indexOf(usrS, pos), str.indexOf(`>`, str.indexOf(usrS, pos)))
			.replace(usrS, ``)
		...
		if (usrM) {
			str = str.replace(`${usrS.concat(snwflk)}>`, `{@${usrM.displayName}}`)
			pos = str.indexOf(`@${usrM.displayName}`, pos) + usrM.displayName.length + 1
		}
	}
	...
	while (str.includes(chS, pos)) {
		snwflk = str
			.slice(str.indexOf(chS, pos), str.indexOf(`>`, str.indexOf(chS, pos)))
			.replace(chS, ``)
