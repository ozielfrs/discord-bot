...
const cF = fs
	...
	.filter(file => file.endsWith(`.js`)) //Command Interactions

// Grab the SlashCommandBuilder#toJSON() output of each command`s data for deployment
for (const f of cF) {
	const cmd = require(`./commands/interaction/${f}`) //Command Interactions
...
;(async () => {
	try {
		...
		for (const g of guildIds) {
			const d = await rest.put(Routes.applicationGuildCommands(clientId, g), {
				body: cmds,
			})
			console.log(
